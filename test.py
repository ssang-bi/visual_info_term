...
def drawPolygon(event, x, y):
    ...
    if event == cv2.EVENT_LBUTTONDOWN and MODE == "Polygon":  # 마우스 좌클릭 이벤트
        if VCOUNT < 2:
            PT.append((x, y))
            cv2.circle(ACTIVE_IMG, (x, y), 1, COLOR)
            if VCOUNT == 1 : cv2.line(ACTIVE_IMG, PT[0], PT[1], COLOR)
        else:
            if checkRad(PT[VCOUNT - 2], PT[VCOUNT - 1], (x, y)) and checkRad((x, y), PT[0], PT[1]):
                if calcLen(x, y, PT[0][0], PT[0][1]) <= 20:
                    cv2.line(ACTIVE_IMG, PT[VCOUNT - 1], PT[0], COLOR)
                    saveImg(ACTIVE_IMG)
                    VCOUNT = -1
                else:
                    PT.append((x, y))
                    cv2.circle(ACTIVE_IMG, (x, y), 1, COLOR)
                    cv2.line(ACTIVE_IMG, PT[VCOUNT - 1], PT[VCOUNT], COLOR)
            else: VCOUNT -= 1
